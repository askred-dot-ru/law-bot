...
### 1.1 Назначение
**LawBot** — Telegram-бот, консультирующий пользователей по законодательству РФ на основе базы из 49 ключевых документов (Конституция, 18 кодексов, 5 ФЗ, 14 постановлений Правительства, 1 распоряжение, 10 приказов в сфере энергетики). Релевантные нормы извлекаются через MCP-сервер векторного поиска и подаются в контекст LLM.
